...
- **Import Markdown**: tombol "Import" di header editor (antara Save Draft dan Publish) — unggah/seret berkas .md (format newscrap: judul H1, metadata Penulis/Sumber Asli/Tanggal Publikasi, gambar pertama = cover). Parser `marked` mengonversi ke rich text Tiptap, excerpt otomatis dari paragraf pertama, source terisi, dan cover di-rehost ke Media Library (bukan hotlink) lewat route anti-SSRF `/api/image-import` + presigned flow. Preview ringkas sebelum apply; hasil tetap bisa diedit sebelum simpan.
